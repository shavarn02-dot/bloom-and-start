...
function corsHeaders(env: Env, request: Request): HeadersInit {
  ...
  return {
    ...
    "access-control-allow-headers": "content-type, authorization, x-api-key, x-user-id, x-client-info, apikey",
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // POST /api/campaigns — Create new campaign (RC-A & RC-B Spec Compliance)
    if (path === "/api/campaigns" && request.method === "POST") {
      ...
      // Hard Cap Enforcement: Max 50 leads for Free Plan
    ...
    if (runMatch && request.method === "POST") {
      ...
      if (searchMode === "smart") {
        ...
        // Filter out listicles/articles
        const dbCompanies = rawCompanies
          .filter((c) => !isListicleOrArticle(c.canonical_name || c.legal_name))
          .slice(0, requestedLimit);

        if (dbCompanies && dbCompanies.length > 0) {
          console.log(`[DB] Smart Search matches found: ${dbCompanies.length} companies matching query "${queryStr}"`);
          ...
          try {
            const leadsToInsert = dbCompanies.map((c: any) => {
              ...
              // Truthful Person/Contact details (No Fabricated "Team" or "Executive")
      ...
      // Create scrape_jobs tracking entry
      ...
      const job = Array.isArray(jobs) && jobs.length > 0 ? jobs[0] : { id: "job_" + Date.now() };
    ...
    // GET /api/campaigns/:id/leads — Fetch leads for specific campaign (Strict Campaign Isolation)
